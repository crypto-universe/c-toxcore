                               ...
                               uint16_t length)
{
    ...
    const size_t packet_size = 1 + sizeof(nospam_num) + length;
    uint8_t packet[ONION_CLIENT_MAX_DATA_SIZE];
    ...
    if (friend_con->status == FRIENDCONN_STATUS_CONNECTED) {
        ...
        return write_cryptpacket(fr_c->net_crypto, friend_con->crypt_connection_id, packet, packet_size, 0);
    }
